# Medicine Schedule

This document outlines the daily medication schedule and frequency for the patient based on the current tracker configuration.

## Schedule by Time of Day

### 🍳 Breakfast (08:00 AM)
- **Dapagliflozin** (Farxiga) - 10 mg
- **Sacubitril / Valsartan** (Entresto) - 24–26 mg
- **Clopidogrel** (Plavix) - 75 mg
- **Furosemide** (Lasix) - 20 mg
- **Tramadol** (Ultram) - 50 mg
- **Methocarbamol** (Robaxin) - 750 mg
- **Metformin** (Glucophage) - 500 mg
- **Glimepiride** (Amaryl) - 5 mg
- **Aspirin** (Low Dose EC) - 81 mg
- **Pravastatin** (Pravachol) - 20 mg

### 🌤️ Afternoon (02:00 PM)
- **Tramadol** (Ultram) - 50 mg
- **Methocarbamol** (Robaxin) - 750 mg

### 🍲 Dinner (06:00 PM)
- **Sacubitril / Valsartan** (Entresto) - 24–26 mg
- **Metoprolol Succinate ER** (Toprol XL) - 25 mg
- **Spironolactone** (Aldactone) - 12.5 mg (½ tablet)

### 🌆 Evening (08:00 PM)
- **Tramadol** (Ultram) - 50 mg
- **Methocarbamol** (Robaxin) - 750 mg

### 🌙 Before Bed (10:00 PM)
- **Zolpidem Tartrate** (Ambien) - 10 mg
- **Atorvastatin** (Lipitor) - 40 mg

### ⚠️ As Needed (PRN)
- **Polyethylene Glycol 3350** (Miralax) - 17 g (At bedtime)
- **Acetaminophen** (Tylenol) - 650 mg (Every 8 hours)

---

## Daily Frequency Count

| Medicine Name | Brand | Times Taken Per Day |
|---------------|-------|---------------------|
| Tramadol | Ultram | **3** |
| Methocarbamol | Robaxin | **3** |
| Sacubitril / Valsartan | Entresto | **2** |
| Dapagliflozin | Farxiga | **1** |
| Clopidogrel | Plavix | **1** |
| Furosemide | Lasix | **1** |
| Spironolactone | Aldactone | **1** |
| Metformin | Glucophage | **1** |
| Glimepiride | Amaryl | **1** |
| Metoprolol Succinate ER | Toprol XL | **1** |
| Zolpidem Tartrate | Ambien | **1** |
| Atorvastatin | Lipitor | **1** |
| Polyethylene Glycol 3350 | Miralax | **PRN** |
| Acetaminophen | Tylenol | **PRN** |
| Aspirin | Low Dose EC | **1** |
| Pravastatin | Pravachol | **1** |
